...
# Back-compat re-export: ClusteringResult moved to tsam.clustering_result in
# the split. Resolve lazily so importing tsam.config doesn't pull in the
# pipeline package (which clustering_result.py needs lazily for .apply()).
def __getattr__(name: str) -> Any:
    if name == "ClusteringResult":
        from tsam.clustering_result import ClusteringResult

        return ClusteringResult
    raise AttributeError(f"module 'tsam.config' has no attribute {name!r}")
