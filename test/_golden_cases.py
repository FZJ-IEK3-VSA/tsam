...
_NEW_KWARGS: dict[str, dict] = {
    ...
    # The one golden not generated by v3.4.2. One of its segments holds exactly
    # two timesteps, which are therefore equidistant, and v3 resolved that tie
    # by floating-point noise — it happened to take the later member because it
    # sorted columns alphabetically and the distance sums then rounded the other
    # way. There is no v3 answer to reproduce, only a v3 coin flip, so this
    # golden records the deterministic choice instead.
    "segmentation_medoid": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="hierarchical"),
        "segments": SegmentConfig(n_segments=8, representation="medoid"),
    },
    "segmentation_maxoid": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="hierarchical"),
        "segments": SegmentConfig(n_segments=8, representation="maxoid"),
    },
    "segmentation_minmax_mean": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="hierarchical"),
        "segments": SegmentConfig(
    ...
    "segmentation_distribution_local": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="hierarchical"),
        "segments": SegmentConfig(n_segments=8, representation=Distribution()),
    },
    # --- period_duration / temporal_resolution axes (only 24h was pinned) ---
    "period_week": {
        "n_clusters": 8,
        "period_duration": 168,
        "cluster": ClusterConfig(method="hierarchical"),
    ...
    # --- Extremes on the methods that never saw them ---
    "averaging_extremes_append": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="averaging"),
        "extremes": ExtremeConfig(method="append", max_value=["Load"]),
    },
    "kmedoids_extremes_append": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="kmedoids"),
        "extremes": ExtremeConfig(method="append", max_value=["Load"]),
    },
    "kmaxoids_extremes_append": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="kmaxoids"),
        "extremes": ExtremeConfig(method="append", max_value=["Load"]),
    },
    # --- Duration curves on the methods that never saw them ---
    "averaging_duration_curves": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="averaging", use_duration_curves=True),
    },
    "contiguous_duration_curves": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="contiguous", use_duration_curves=True),
    },
    "kmedoids_duration_curves": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="kmedoids", use_duration_curves=True),
    },
    "kmaxoids_duration_curves": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="kmaxoids", use_duration_curves=True),
    },
    # --- Duration-curve interactions. The candidate matrix is reshaped
    # per-column on this path, so pairing it with the options that change the
    # candidate width or the represented set is where it is most likely to
    # drift. ---
    "duration_curves_period_sums": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(
            method="hierarchical", use_duration_curves=True, include_period_sums=True
        ),
    },
    "duration_curves_extremes": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="hierarchical", use_duration_curves=True),
        "extremes": ExtremeConfig(method="append", max_value=["Load"]),
    },
    "duration_curves_segmentation": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="hierarchical", use_duration_curves=True),
        "segments": SegmentConfig(n_segments=8),
    },
    # --- include_period_sums interactions (only the bare case was pinned) ---
    "period_sums_weighted": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="hierarchical", include_period_sums=True),
        "weights": {"Load": 5.0, "GHI": 1.0, "T": 1.0, "Wind": 1.0},
    },
    "period_sums_segmentation": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="hierarchical", include_period_sums=True),
        "segments": SegmentConfig(n_segments=8),
    ...
    # --- round_decimals=0 (only 2 was pinned) ---
    "round_zero": {
        "n_clusters": 8,
        "period_duration": 24,
        "cluster": ClusterConfig(method="hierarchical"),
...
# Metadata shared by the axis-completion cases, which are all pinned on a
# single dataset. ``kmedoids`` is truncated because its exact solver scales
# badly; ``kmeans``/``kmaxoids`` are seeded and compared loosely because they
# are not deterministic across BLAS builds.
_DETERMINISTIC_TESTDATA = {
    "seed": None,
    "only_datasets": ["testdata"],
    "max_timesteps": None,
    "rtol": 1e-10,
}
_STOCHASTIC_TESTDATA = {
    "seed": 42,
    "only_datasets": ["testdata"],
    "max_timesteps": None,
    "rtol": 1e-05,
...
_META: dict[str, dict] = {
    ...
    "segmentation_distribution_local": _DETERMINISTIC_TESTDATA,
    "period_week": _DETERMINISTIC_TESTDATA,
    "period_48h": _DETERMINISTIC_TESTDATA,
    "period_duration_string": _DETERMINISTIC_TESTDATA,
    "temporal_resolution_explicit": _DETERMINISTIC_TESTDATA,
    "averaging_extremes_append": _DETERMINISTIC_TESTDATA,
    "kmedoids_extremes_append": _KMEDOIDS_TESTDATA,
    "kmaxoids_extremes_append": _STOCHASTIC_TESTDATA,
    "averaging_duration_curves": _DETERMINISTIC_TESTDATA,
    "contiguous_duration_curves": _DETERMINISTIC_TESTDATA,
    "kmedoids_duration_curves": _KMEDOIDS_TESTDATA,
    "kmaxoids_duration_curves": _STOCHASTIC_TESTDATA,
    "duration_curves_period_sums": _DETERMINISTIC_TESTDATA,
    "duration_curves_extremes": _DETERMINISTIC_TESTDATA,
    "duration_curves_segmentation": _DETERMINISTIC_TESTDATA,
    "period_sums_weighted": _DETERMINISTIC_TESTDATA,
    "period_sums_segmentation": _DETERMINISTIC_TESTDATA,
    "samemean_unweighted": _DETERMINISTIC_TESTDATA,
    "samemean_extremes": _DETERMINISTIC_TESTDATA,
