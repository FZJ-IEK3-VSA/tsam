...
@pytest.mark.parametrize(
    ...
    [
        ...
        "wide",
        "wide_segmented",
        pytest.param("production", marks=pytest.mark.large),
        pytest.param("production_segmented", marks=pytest.mark.large),
    ],
)
def test_disaggregate(benchmark, shape):
    ...
    Two axes matter and both are covered: segmented results expand through
    different code than unsegmented ones, and the cost scales with the size of
    the *output* — the production shapes expand to two years x 400 columns,
    seven million cells, which is where a model of the issue #49 shape lands.
    """
    if shape == "small":
        data, n_clusters, segments = _testdata(), N_CLUSTERS, None
    elif shape.startswith("production"):
        data, n_clusters = _tile_years(_wide_columns(400), 2), 40
        segments = SegmentConfig(n_segments=12) if shape.endswith("segmented") else None
    else:
