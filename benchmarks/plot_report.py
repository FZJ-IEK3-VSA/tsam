...
def _frame(runs: list[Path], labels: list[str], stat: str = "min"):
    """Tidy long frame, one row per (run, benchmark).

    ``min`` by default rather than ``median``: contention only ever *adds*
    time, so on a machine that is not perfectly idle the fastest observed run
    is the closest estimate of the true cost, and both arms are treated
    identically. Pass ``--stat median`` when the machine is quiet and the
    spread itself is of interest.
    """
    df, unit = load_long_df(runs, metric="time", stat=stat, labels=labels)
...
def _scaling_figure(df, labels: list[str]) -> go.Figure:
    ...
    for i, (func, xdim, _title) in enumerate(present):
        ...
        for j, label in enumerate(labels):
            arm = (
                sub[sub["snapshot"] == label].groupby(xdim, as_index=False)["ms"].min()
            )
            if arm.empty:
                continue
...
def _attribution_rows(df, labels: list[str]) -> list[tuple[str, float, float, float]]:
    ...
    wide = df.pivot_table(index="id", columns="snapshot", values="ms", aggfunc="min")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="per-run statistic; min is robust to background load",
    )
    ...
    df = _frame(args.runs, labels, args.stat)
    ...
    args.out.write_text(
        ...
        f"{moved} moved by more than 5% ({args.stat} of each run). Log-log axes: a straight line is a power law "
